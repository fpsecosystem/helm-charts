...
2. **MariaDB instance not ready**:
   ```bash
   # Check MariaDB status
   kubectl get mariadb -n mariadb-system
   ```

3. **Permission denied errors**:
   - Verify the MariaDB instance is accessible
   - Check if the root password is correct
   - Ensure the operator has proper RBAC permissions

4. **Chart dependency issues**:
   ```bash
...
# Get MariaDB logs
kubectl logs -n mariadb-system mariadb-instance-0

# Check created resources
kubectl get database,user,grant -n your-namespace
```

## Best Practices

1. **Use generated passwords** for better security
2. **Limit user privileges** to only what's needed
3. **Use specific table grants** when possible
4. **Monitor resource creation** with proper labels
...
helm lint your-app/
helm template your-app your-app/ --dry-run
helm install --dry-run your-app your-app/
```
